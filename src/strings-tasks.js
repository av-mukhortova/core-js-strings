...
function removeLastOccurrences(str, value) {
  const first = str.lastIndexOf(value);
  if (first < 0) return str;
  const last = first + value.length;
  return str.substring(0, first) + str.substring(last, str.length);
}
...
function countVowels(str) {
  ...
  if (str) {
    for (let i = 0; i < str.length; i += 1) {
      if (vowels.indexOf(str[i].toLowerCase()) >= 0) count += 1;
    }
  }
  return count;
}
...
function isPalindrome(str) {
  const originalStr = str.toLowerCase().replace(/[^a-zа-яё]/gi, '');
...
function findLongestWord(sentence) {
  ...
  return words[wordLengths.indexOf(longest)];
}
...
function invertCase(str) {
  return str
    .split('')
    .map((s) => (s.toUpperCase() === s ? s.toLowerCase() : s.toUpperCase()))
    .join('');
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((letter) =>
      normal.indexOf(letter) >= 0 ? cipher[normal.indexOf(letter)] : letter
    )
    .join('');
}
/**
